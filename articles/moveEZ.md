...
- `hulls`: logical; when `TRUE` convex hulls summarise group spread at
  ...
  In all three functions a hull requires at least three non-collinear
  observations per group per time level — groups with fewer are plotted
  as points automatically.
