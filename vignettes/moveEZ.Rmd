...
- `hulls`: logical; when `TRUE` convex hulls summarise group spread at each
  time level; when `FALSE` individual sample points are displayed. In all
  three functions a hull requires at least three non-collinear observations
  per group per time level — groups with fewer are plotted as points
  automatically.
...
anim_save("vignettes/anim1.gif", animation = last_animation())
...
- `reflect`: specifies the axis whose direction is reversed — `"x"`
  ...
  `biplotEZ::reflect()`, with each entry corresponding to a level in
  `align.time`. Both arguments accept vectors when alignment is needed at
...
x-axis is reversed. This is a sign indeterminacy artifact, not a genuine structural change.
...
Reversing the x-axis at 1950 with `reflect = "x"` restores visual continuity
